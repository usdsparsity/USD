...
class Sparse_find_mix_from_dense(autograd.Function):
    ...
    def forward(ctx, weight, N_intermediate, M, decay, learned_threshold, normalized_factor,layer_name,print_flag,data_layout,apply_penalty):
        ...
        #assert weight.is_cuda, "Tensor must be on Cuda"
        #assert length % M == 0, "Total number of elements mustb be divisible by M"
        ...
        _, topK_indices = torch.topk(weight_temp.abs(), k=M-N_intermediate, dim=1, largest=False, sorted=True)
        #index = torch.argsort(weight_temp.abs(), dim=1)[:, :int(M-N_intermediate)] # indicate which will be zeros (pruned) 
        #
        #w_b = torch.ones(weight_temp.shape, device=weight_temp.device)
        #w_ b = w_b.scatter_(dim=1, index=index, value=0) #.reshape(weight.shape) #
...
class Sparse(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, N, M, decay, eval=False):
        if not eval:
            ctx.save_for_backward(weight)
            ctx.M = M
            ctx.N = N 
        ...
        assert length % M == 0, "Total number of elements mustb be divisible by M"
        group = int(length/M)
        reshaped = weight.view(group, M)
        _, topK_indices = torch.topk(reshaped.abs(), k=M-N, dim=1, largest=False, sorted=True)
...
class Sparse_NHWC(autograd.Function):
    ...
    def forward(ctx, weight, N, M, decay ):
        ...
        weight_t = weight.permute(0,2,3,1)
        weight_temp = weight_t.reshape(group, M)
        _, topK_indices = torch.topk(weight_temp.abs(), k=M-N, dim=1, largest=False, sorted=True)
        mask = torch.ones_like(weight_temp, dtype=weight_temp.dtype)
        mask.scatter_(1, topK_indices, 0)
...
class SparseConv(nn.Conv2d):
    ...
    def smallest_among_survival(self):
        ...
        if self.layout == 'NCHW' or self.k_ == 1:
            weight_temp = self.weight.reshape(group, M)
        ...
        weights_survival = mask * weight_temp
        ...
        smallest_of_survival,_ = torch.min(smallest_of_survival_,dim = 1)
        smallest_of_survival_col = smallest_of_survival.reshape(smallest_of_survival.numel(),1)
        assert smallest_of_survival_col.is_cuda, "Tensor smallest_of_survival_col must be on Cuda"
    ...
    def update_learned_sparsity_alpha(self):
        ...
        mask = torch.ones_like(weight_temp, dtype=weight_temp.dtype)
        mask.scatter_(1, topK_indices, 0)
        weights_pruned = (1-mask) * weight_temp
        largetest_among_pruned,_ = torch.max(weights_pruned,dim = 1)
        ...
        weights_pruned = mask * weight_temp
        ...
        if N==M:
            self.learned_threshold_m = torch.full(initial_values_shape, threshold_all)
            assert self.learned_threshold_m.is_cuda, "Tensor self.learned_threshold_m must be on Cuda"
        else:
            pass
    ...
    def update_learned_sparsity(self):
        ...
        assert self.smallest_survival.is_cuda, "Tensor self.smallest_survival must be on Cuda"
        assert self.learned_threshold.is_cuda, "Tensor self.learned_threshold must be on Cuda"
    ...
    def intialize_threshold_with_average_lowest(self):
        ...
        _, topK_indices = torch.topk(weight_temp.abs(), k=M-N, dim=1, largest=False, sorted=True)
        mask = torch.zeros_like(weight_temp, dtype=weight_temp.dtype)
        mask = mask.scatter_(dim=1, index=topK_indices, value=1) #.reshape(self.weight.shape) #

        weights_lower = weight_temp * mask
        ...
        return initial_values, self.weight
    ...
    def check_num_survival_parameters(self):
        ...
        if self.layout == 'NCHW' or self.k_ == 1:
            mask_ = (self.weight.abs().reshape(group, M) >= self.smallest_survival) * 1.0

        elif self.layout == 'NHWC': #
            weight_t = self.weight.permute(0,2,3,1)
            mask_ = (weight_t.abs().reshape(group, M) > self.smallest_survival) * 1.0
    ...
    def calculate_mask_w_survival(self, w_current):
        ...
        group = int(length/M)
        if self.layout == 'NCHW' or self.k_ == 1: 
            weight_current = w_current.reshape(group, M)
            weight_previous = self.w_at_t_minus_1.abs().reshape(group, M)
        elif self.layout == 'NHWC': 
            weight_t = w_current.permute(0,2,3,1)
            weight_current = weight_t.reshape(group, M)
            weight_previous = self.w_at_t_minus_1.abs().permute(0,2,3,1).reshape(group, M)
        weight_current = weight_current.abs()
...
class SparseLinear(nn.Linear):
    ...
    def update_learned_sparsity(self):
        ...
        www = None
        if N==M:
            self.learned_threshold_m,_  = self.intialize_threshold_with_average_lowest()   #self.smallest_survival
        else:
