...
class SparseLinear(nn.Linear):
    ...
    def inference_sparse(self, x: torch.Tensor):
        ...
        if len(original_shape) == 3:
            ...
            result_2d = torch.sparse.mm(self.pruned_weight_sparse, x.reshape(-1, embed_dim).t()).t() + self.bias
            return  result_2d.view(batch_size, seq_len, -1)
        else:
            result = torch.sparse.mm(self.pruned_weight_sparse, x.t()).t() + self.bias
            return result
