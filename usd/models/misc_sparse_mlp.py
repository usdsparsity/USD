...
from devkit.sparse_ops.sparse_ops import SparseLinear
...
class SparseMLP(torch.nn.Sequential):
    ...
    def __init__(
        ...
        for hidden_dim in hidden_channels[:-1]:
            layers.append(SparseLinear(in_dim, hidden_dim, bias=bias, N = N, M = M, search = search))
            if norm_layer is not None:
        ...
        layers.append(SparseLinear(in_dim, hidden_channels[-1], bias=bias, N = N, M = M, search = search))
        layers.append(torch.nn.Dropout(dropout, **params))
